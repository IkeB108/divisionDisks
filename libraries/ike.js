...
if (pointX >= x &&         // right of the left edge AND
    ...
    pointY <= y + h) {    // above the bottom
        return true;
}
return false;
}

function rectAfterCollisions(actorRect, collisionRects, xVelocity, yVelocity){
  ...
  for(let i in collisionRects){
    ...
    if(collideRectRectNoEdges(actorRect.x + xVelocity, actorRect.y, actorRect.w, actorRect.h, cr.x, cr.y, cr.w, cr.h) ){
      ...
      if(xVelocity > 0) newActorRect.x = cr.x - actorRect.w;
      if(xVelocity < 0) newActorRect.x = cr.x + cr.w;
      break;
    }
  }
  ...
  for(let i in collisionRects){
    ...
    if(collideRectRectNoEdges(newActorRect.x, actorRect.y + yVelocity, actorRect.w, actorRect.h, cr.x, cr.y, cr.w, cr.h) ){
      newYCollides = true;
      //Use the direction of the collision to snap actor to edge of collisionRect
      if(yVelocity > 0) newActorRect.y = cr.y - actorRect.h;
      if(yVelocity < 0) newActorRect.y = cr.y + cr.h;
      break;
    }
  }
